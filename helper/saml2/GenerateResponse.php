...
use MSTUSI\Helper\Factory\ResponseHandlerFactory;
...
class GenerateResponse implements ResponseHandlerFactory
{
	...
	function buildAssertion($response_params)
	{
		...
		//Build AttrStatment
		$attrStatement = $this->buildAttrStatement();
	...
	/**
	 * ALl the three attributes need to be send to the Talent LMS as SP, They are required attributes.
	 * Exception thrown for any of the empty value. 
	 */
	function buildAttrStatement()
	{
		$attrStatement = $this->xml->createElement('saml:AttributeStatement');

		$attrs = $this->buildAttribute("firstname","first_name");
		if(is_null($attrs)) throw new InvalidSSOUserException();
		$attrStatement->appendChild($attrs);

		$attrs = $this->buildAttribute("lastname" ,"last_name");
		if(is_null($attrs)) throw new InvalidSSOUserException();
		$attrStatement->appendChild($attrs);
		
		$attrs = $this->buildAttribute("email" ,"user_email");
		if(is_null($attrs)) throw new InvalidSSOUserException();
		$attrStatement->appendChild($attrs);
	...
	{
		$value = $this->current_user->$attrValue;
		if (empty($value)) $value = get_user_meta($this->current_user->ID, $attrValue, TRUE);
		$value = apply_filters('generate_saml_attribute_value',$value,$this->current_user,$attrName,$attrValue);

		if(empty($value)) return null;
		return $this->createAttributeNode($value,$attrName);
	}

	function createAttributeNode($value,$attrName)
	{
		$attrs = $this->xml->createElement('saml:Attribute');
		$attrs->setAttribute('Name',$attrName);
		$attrs->setAttribute('NameFormat','urn:oasis:names:tc:SAML:2.0:attrname-format:unspecified');
		if(is_array($value))
		{
			foreach ($value as $key => $val)
			{
				$val = apply_filters('modify_saml_attr_value', $val);
				$attrsValueElement = $this->xml->createElement('saml:AttributeValue',htmlspecialchars($val));
				$attrs->appendChild($attrsValueElement);
			}
		}
		else
		{
			$value = apply_filters('modify_saml_attr_value', $value);
			$attrsValueElement = $this->xml->createElement('saml:AttributeValue',htmlspecialchars($value));
			$attrs->appendChild($attrsValueElement);
		}
		return $attrs;
	}
