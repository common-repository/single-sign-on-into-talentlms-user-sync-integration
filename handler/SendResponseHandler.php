...
use MSTUSI\Helper\Factory\ResponseDecisionHandler;
...
final class SendResponseHandler extends BaseHandler
{
	...
	{
		if(MSTUSI_DEBUG) MoIDPUtility::mo_debug("Generating Login Response");
		...
		if(MSTUSI_DEBUG) MoIDPUtility::mo_debug( "Login Response generated: " . $response);
	...
	public function getSAMLResponseParams($args,$current_user)
	{
		...
		$issuer 	= get_site_option('mo_idp_entity_id') ?  get_site_option('mo_idp_entity_id') : MSTUSI_URL;
	...
	{
		if(MSTUSI_DEBUG) MoIDPUtility::mo_debug("Sending SAML Login Response");
		...
		<html>
			...
			<form id="responseform" action="'.esc_url($acs_url).'" method="post">
				<input type="hidden" name="SAMLResponse" value="'.esc_attr(htmlspecialchars($saml_response)).'" />';
        if($relayState!="/") {
            echo '<input type="hidden" name="RelayState" value="' . esc_html($relayState) . '" />';
        }
        echo '</form>
			</body>
		<script>
	...
	{
		if(MSTUSI_DEBUG) MoIDPUtility::mo_debug("Sending WS-FED Login Response");
        echo'
		<html>
			<head>
				<meta http-equiv="cache-control" content="no-cache">
			...
			<body>
				<form id="responseform" action="'.esc_url($acs_url).'" method="post">
					<input type="hidden" name="wa" value="'.esc_html($wa).'" />
			
					<input type="hidden" name="wresult" value="'.esc_attr(htmlentities($wsfed_response)).'" />
					<input type="hidden" name="wctx" value="'.esc_html($wctx).'" />';
        echo '	</form>
			</body>
			<script>
				document.getElementById(\'responseform\').submit();	
